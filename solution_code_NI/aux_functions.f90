...
module aux_functions
    ...
    function vDefault(xIn) result(vDefault1) ! Value of default (Y_t, B_{t},m_t)
        ...
        ! if(ccounter .EQ. 2) then 
        !     vDefault1 = sov_utility(ydef(xIn(1)))
        ! else 
            xTransformed(1) = (xIn(1) - yL)/(yH - yL)
            xTransformed(2) = (xIn(2) - bL)/(bH - bL)
            ...
            ! vDefault1 = VD_std*GPR_approx(xTransformed,VD_gprCoeff,VD_kernCoeff,scaledGridPointsM) + VD_mean  
        ! end if 
    ...
    ! Equilibrium repayment/issuance price (Y_t, B_{t+1})
    ...
    function q(xIn) result(q1) ! Equilibrium repayment/issuance price (Y_t, B_{t+1})
        implicit none 
        ...
        q1 = 1.0/(1.0 + exp(-q_std*GPR_approx(xTransformed,q_gprCoeff,q_kernCoeff,scaledGridPointsX(1:2,:),2)-q_mean))

    end function q 

    ! Equilibrium default price (Y_t, B_{t+1}, m_t)
    ! Required to be bounded in [0,1] by using a logit-structure 
    ...
    ! Off-equilibrium RN repayment/issuance price (Y_t, B_{t+1})
    ! Required to be bounded in [0,1] by using a logit-structure 
    ! This is valid because coupons are set to risk-free rate -> risk-free price = 1
    function qRN(xIn) result(q1) ! Equilibrium repayment/issuance RN price (Y_t, B_{t+1})
        implicit none 
        ...
        q1 = 1.0/(1.0 + exp(-qRN_std*GPR_approx(xTransformed,qRN_gprCoeff,qRN_kernCoeff,scaledGridPointsX(1:2,:),2)-qRN_mean))
    ...
    ! The general, 1-D optimization routine using golden-section search. It applies to both the forecaster's problem and the 
    ! sovereign Bellman
    ...
    function repayBellman(issued_b,current_y,current_b) result(repayBellman1)
        ...
        double precision, dimension(quad_size) :: z_gc_points, yvec, mvec
        double precision, dimension(nDims-1) :: tom_repay_state, curr_q_state
        double precision, dimension(nDims) :: tom_default_state, curr_def_state
        ...
        if( (q(curr_q_state) < min_q) .AND. (current_b < 1.0e-6 ) ) then 
    ...
    function new_q(current_y,issued_b) result(new_q1) 
        implicit none 

        double precision, intent(in) :: current_y, issued_b ! current state
        ...
        do while( qH - qL > opt_tol )
            ...
            q_guess = .5*(qL + qH)
            curr_gap = q_gap( q_guess, current_y,issued_b)
    ...
    function q_gap(curr_q,current_y,issued_b) result(q_gap1)
        implicit none

        double precision, intent(in) :: curr_q, current_y, issued_b ! current state
        double precision :: zL1, zH1, ytempObj_top, ytempObj_bottom, mtempObj_top, mtempObj_bottom, & 
            MU_num, MU_denom, lmu_top, lmu_bottom, defInd, tom_qd, marg_return
        double precision :: q_gap1

        integer :: ix, im, iy ! Loop counters

        double precision, dimension(quad_size) :: z_gc_points, mvec_top, mvec_bottom, yvec_top, yvec_bottom
        double precision, dimension(nDims-1) :: tom_q_state, tom_borr_state
        double precision, dimension(nDims) ::   tom_qd_state 
        ...
        ! can be more expensive than storing scalars
            ...
            tom_q_state(2) = APol(tom_borr_state)
    ...
    function qd_gap(curr_qd,current_y,current_b,current_m) result(qd_gap1)
        ...
        double precision :: zL1, zH1, ytempObj_top, ytempObj_bottom, MU_num, & 
        ...
        double precision, dimension(quad_size) :: z_gc_points, yvec_top, yvec_bottom
        double precision, dimension(nDims-1) :: tom_q_state, future_borrow_state
        double precision, dimension(nDims) :: tom_qd_state
        ...
        do iy=1,quad_size 
            ...
            tom_q_state(2) = APol(future_borrow_state)
    ...
    function new_qRN(current_y,issued_b) result(new_q1)
        implicit none
    ...
    subroutine drawInputs(trainingInputsX,trainingInputsM)
        ...
        ! First, build random index lists for dimensions 2 and 3
        ! call scramble(nInputs, b_inds)

        ! ! b-grid points are equidistant (typica under uniform) because b is endogenous
        ! do i=1,nInputs
        !     trainingInputsX(2,i) = bL + (bH-bL)/(DBLE(nInputs)-1.0)*DBLE(b_inds(i)-1)
        !     trainingInputsM(2,i) = bL + (bH-bL)/(DBLE(nInputs)-1.0)*DBLE(b_inds(i)-1)
        ! end do 

        ! b-grid points are equidistant (typica under uniform) because b is endogenous
        ! do i=1,nInputs
        !     trainingInputsX(2,i) = bL + (bH-bL)/(DBLE(nInputs)-1.0)*DBLE(i-1)
        !     trainingInputsM(2,i) = bL + (bH-bL)/(DBLE(nInputs)-1.0)*DBLE(i-1)
        ! end do 
    ...
    subroutine simulated_moments(key_moments) 
        ...
        double precision, dimension(Nsimul+1) :: sim_y, sim_b, sim_m, sim_x, sim_rho, sim_q, & 
            ...
            sim_y_issuing, sim_info_issuing, sim_ann_sprdRN, sim_ann_sprd_issuingRN
        ...
        double precision, dimension(nDims) :: curr_def_state
        double precision, dimension(nDims-1) :: curr_repay_state, curr_info_state, curr_price_state
        ...
        open(3,file="simulations.txt")
            ...
            do t=1,Nsimul 
                ...
                write(3,"(10F9.6)") sim_y(t), sim_b(t), sim_m(t), sim_x(t), sim_access(t), sim_q(t), sim_ann_sprd(t), & 
                    sim_rho(t), sim_ann_sprdRN(t), sim_issuing(t)
    ...
    function negLL_q(log_hyper_params) result(negLL1)
        ...
        negLL1 = negLL(log_hyper_params,tset_q,1)
    ...
    function negLL_qRN(log_hyper_params) result(negLL1)
        ...
        negLL1 = negLL(log_hyper_params,tset_qRN,1)
    ...
    function kernelFunc(x,xprime,ttheta_vec,dim_size) result(kernelFunc_1)
        ...
        ! ! Now apply the Matern 5/2 formula
        ! kernelFunc_1 = ( 1.0 + sqrt(5.0)*scaled_d + 5.0*scaled_d**2.0/3.0)*exp(-sqrt(5.0)*scaled_d + h)
    ...
    function GPR_approx(x,AA,kern_params,xx,dim_size) result(GPR_approx1)
        ...
        integer, intent(in) :: dim_size 
        double precision, intent(in), dimension(dim_size) :: x 
        double precision, intent(in), dimension(dim_size,nInputs) :: xx
        double precision, intent(in), dimension(2) :: kern_params !
        double precision, intent(in), dimension(nInputs) :: AA ! Coefficients
