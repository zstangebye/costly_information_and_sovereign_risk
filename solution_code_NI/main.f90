...
program main
    ...
    ! integer :: ccounter ! Keeps track of iterations

    ! Temporary variables used in solution
    double precision :: ytod, bissued, btod, xtod, mtod, optBorrowingSoln, bellmanValue
    double precision, dimension(nInputs,nInputs) :: K_VR, K_A, K_VD, K_q, K_qd, K_qRN, K_qdRN, eyeN, ttempK
    ...
    nii  = int(DBLE(nInputs-1)/DBLE(nproc))+1 ! Number of inputs handled by each processor (econ optimization)
    ...
    nii2  = int(DBLE(nKernCoeffsDraws-1)/DBLE(nproc))+1 ! Number of inputs handled by each processor (hyperparam optimization)
    ...
    nii3  = int(DBLE(nKernCoeffsDraws*2-1)/DBLE(nproc))+1 ! Number of inputs handled by each processor (draw hyperparams)
    ...
    ! We  set the GPR coefficients to zero, which corresponds to
    ! all functions having initial values of zero, as they would in the
    ! final period in a finite-horizon game
    VR_gprCoeff = 500.0
    A_gprCoeff = 0.0
    I_gprCoeff = 0.0
    VD_gprCoeff = 500.0
    ...
    ! Notice that if we not separately seed the random processes yet, they will give every processor the same
    ! grid points
    ...
    ! Next, draw random uniforms for the hyperparameter optimization later. Here we want the random draws to be 
    ! different across processors
 
    call random_number(seed) 
    seed = int(seed*1000)*(id+1) 
    call random_seed(put=int(seed))

    ! Setting these two zero implies that calling their likelihood just delivers log(det(Ssigma)) and Ssigma^(-1), which depends only 
    ! on the hyperparameters we're evaluating here. This way we can make sure none of the initial combinations of 
    ! hyperparameters and training inputs imply infinite likelihoods. Combine this with a gradient-free Nelder-Mead optimization 
    ! and we'll get a smooth optimization routine later
    tset_q = 1.0 
    ...
    if( id .EQ. 0) then 
        ...
        do i = 1, nKernCoeffsDraws 
            ...
            do while( .NOT. found_hyper_params)
                ...
                if( negLL_q(temp_coeff_vec) < 9.9e9 ) then 
                    found_hyper_params = .TRUE.
                end if 

            end do 
            val4agg( i ) = negLL_q(temp_coeff_vec)
    ...
    ! Now reformulate the random draws into blocks and scale them into the appropriate windows
    i1 = 1
    do i=1,nKernCoeffsDraws 
        do j=1,2 
    ...
    ! Now every processor has the block! 
    ...
    do while((ddist > conv_tol) .AND. (ccounter < max_iters))
        ...
        ccounter = ccounter + 1
            ...
            do i=itop,iend 
                ...
                ! val1(i-itop+1) = new_q(ytod,bissued,xtod) 
                if( ccounter > 1) then 
                    val1(i-itop+1) = -log(1.0/max(new_q(ytod,bissued), 1.0e-6) - 1.0)
                else 
                ...
                ! val2(i-itop+1) = new_qd(ytod,btod,mtod) 
                if( ccounter > 1) then 
            ...
            ! Next, we de-mean the point estimates
            ...
            call createK(K_q, q_kernCoeff,1)
            ...
            do i=itop,iend 
                ...
                ! val1(i-itop+1) = new_qRN(ytod,bissued,xtod)
                if(ccounter > 1) then 
                    val1(i-itop+1) = -log(1.0/max(new_qRN(ytod,bissued),1.0e-6) - 1.0)
                else 
                    val1(i-itop+1) = -log(1.0/(1.0e-6) - 1.0)
                end if 
                ...
                ! val2(i-itop+1) = new_qdRN(ytod,btod,mtod)
                if(ccounter > 1) then 
            ...
            ! Update the gpr coefficients, which automatically updates the policy itself
            call createK(K_qRN, qRN_kernCoeff,1)
        ...
        do i=itop,iend 
            ...
            if( (bH_local > test_point(2)) .AND. (q(test_point(1:2)) < min_q) ) then 
            ...
            if( (q(test_point(1:2)) < min_q) .AND. (gridpointsX(2,i) < 1.0e-6 ) ) then 
                optBorrowingSoln = bL + 1.0e-6
            end if

            ! ! Here, we've violated the issuance at low prices constraint
            if( (bellmanValue .ge. VH-1.0e-6) .OR. (bellmanValue .LE. VL+1.0e-6) ) then 
            ...
            ! write(3,*) bH_local !optBorrowingSoln
            ! val1(i-itop+1) = optBorrowingSoln
            ! val1(i-itop+1) = optBorrowingSoln - gridPointsX(2,i)*(1.0-llambda)
            val1(i-itop+1) = -log( (bH - bL)/min(max(optBorrowingSoln - bL,1.0e-6),bH-bL-1.0e-6) - 1.0)

            ! No transformation of value functions necessary. GP operates on them directly
            ...
            ! Finally, if this is a state of default, just set the policy function to be the 
            ! current level of debt instead. This will help with extrapolation and it's also 
            ! technically correct since the country cannot change its position in default 
            ! unless a deal is presented to it
            ! if(val2(i-itop+1) < val3(i-itop+1)) val1(i-itop+1) = (1.0-llambda)*btod
            ! if(val2(i-itop+1) < val3(i-itop+1)) val1(i-itop+1) = -log( (bH - bL)/( min(btod,btod-1.0e-6) - bL) - 1.0)
        ...
        ! Next, we de-mean and standardize the outputs
        ...
        ! Now check convergence by evaluating again
        if( id .EQ. 0) then 
            do i=1,N_validate 
