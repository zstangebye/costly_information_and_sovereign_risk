...
module global_vars  
    ...
    double precision, parameter :: CRRA_LEND = 0.92 ! Lender risk-aversion
    double precision, parameter :: bbeta = 0.9653 ! Country impatience
    ...
    ! Debt parameters 
    double precision, parameter :: r = .01 ! Quarterly risk-free rate (a bit less than Arellano)
    ...
    double precision, parameter :: min_q = .7 ! 70% of risk-free price (always one here)

    ! Information cost 
    double precision, parameter :: kkappa = 1.0e-5 ! Information cost

    ! Default cost parameters
    double precision, parameter :: ythresh = .83 ! break below which default is free
    double precision, parameter :: MP = .15 ! maximum proportional cost (applied at yH due to standard curvature!)
    ...
    double precision, parameter :: VL = -100.0, VH = -1.0e-6

    ! Debt grid measures
    double precision, parameter :: b_thresh = 0.4
    ...
    double precision, parameter :: stds_out = 3.0 ! How many stdevs out to set bounds
    ...
    double precision, dimension(2) :: VD_kernCoeff, q_kernCoeff, qd_kernCoeff, &
        qRN_kernCoeff, qdRN_kernCoeff ! GPR kernel parameters (3 dims)
    double precision, dimension(2) :: VR_kernCoeff, A_kernCoeff, I_kernCoeff ! GPR kernel parameters (2 dims)
    double precision, dimension(nInputs) :: VR_gprCoeff, VD_gprCoeff, A_gprCoeff, I_gprCoeff, &
        q_gprCoeff, qd_gprCoeff, qRN_gprCoeff, qdRN_gprCoeff ! GPR coefficients 
    double precision, dimension(nInputs) :: tset_VR, tset_VD, tset_A, tset_I, tset_q, tset_qd, &
        tset_qRN, tset_qdRN, tset_VRz, tset_VDz, tset_Az, tset_Iz, &
        tset_qz, tset_qdz, tset_qRNz, tset_qdRNz
    double precision :: VR_mean, VD_mean, A_mean, I_mean, q_mean, qd_mean, qRN_mean, qdRN_mean, & 
        VR_std, VD_std, A_std, I_std, q_std, qd_std, qRN_std, qdRN_std
